<?php
    class Template{
        ...
        function viewsku($template = NULL, $data = NULL){
            ...
            // Bagian $data['head'] untuk memanggil file head.php dari folder Admin/_Template/
        // ['head'] data yang kita panggil dari file template.php dari folder Admin/_Template/
           $data['head'] = $this->_ci->load->view('staff/_template/head', $data, TRUE);
           
           $data['topbar'] = $this->_ci->load->view('staff/_template/topbar', $data, TRUE);

           $data['sidebar'] = $this->_ci->load->view('staff/_template/sidebar', $data, TRUE);

           $data['isi'] = $this->_ci->load->view($template, $data, TRUE);

           $data['content'] = $this->_ci->load->view('staff/_template/content', $data, TRUE);

           $data['footer'] = $this->_ci->load->view('staff/_template/footer', $data, TRUE);

           $data['js'] = $this->_ci->load->view('staff/_template/js', $data, TRUE);

           echo $data['template'] =$this->_ci->load->view('staff/_Template/Template', $data, TRUE);
            }
        }
    }